...
<!-- TODO: 在此补充"如何从 OneDrive 网页端生成共享链接"的图文操作说明 -->
<!-- 示例步骤（供参考，请替换为你的实际说明）：
1. 打开 OneDrive 网页版，右键目标文件 → 「共享」→ 「获取链接」
2. 将链接权限设置为「任何人可查看」（不要设置密码）
3. 复制形如 https://1drv.ms/v/c/{cid}/{token} 的嵌入链接，粘贴到本站「OneDrive 链接」模式解析
-->
...
- **可配置间隔定时自动刷新**：Worker 配置了 Cron Trigger（每 5 分钟触发 `scheduled` 事件），按设置中的「OneDrive 自动刷新间隔」决定实际刷新周期，遍历所有含 OneDrive 锚点（原始共享链接 + driveId/itemId）的媒体条目，重新获取 BadgerAuth 并解析最新 `tempauth` 直链回写 KV，在令牌过期前完成续期。由于网站缓存了解析结果，仅使用网站外链时即使解析链过期外链仍可用，可将间隔调大以减少 OneDrive 请求。设置页会结合「缓存 TTL」动态显示最长缓存时间，供参考三档时间：**最短 1 小时**（保持解析链不过期）、**不使用解析链（仅用网站外链）时推荐设为最长缓存时间**（几乎不再产生 OneDrive 请求）、**最长为网站最长缓存时间**。距上次刷新剩余时间 ≤ 310 秒（5 分钟检查周期 + 10 秒缓冲）时触发自动解析并更新时间，规避网络延迟造成的空档。
...
6. 在「设置」→「缓存与限制」中可调整「缩略图媒体源」与「预览媒体源」（上游媒体源 / 网站媒体源），保存后即时生效
...
│   └── lib/
│       ├── onedrive.js   # OneDrive 新旧格式解析：base64url / 跳转链 / BadgerAuth / 递归遍历
